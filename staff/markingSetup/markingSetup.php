<?php
    ...
    if (empty($_POST["printableVersion"])) 
    {
        ...
        $html.="\n<head><title>Marking Setup Progress</title></head>\n<body>\n<form action='markingSetup.php' name='markingSetup' method='post'>";
        ...
        $dom=new DOMDocument();
        $dom->load($markingSetupFile);
        $markingXpath = new DOMXPath($dom);    
        ...
        $dom->Load($studentChoices);
        $xpathStudentChoices = new DOMXPath($dom);  
        ...
        $domProjects=new DOMDocument();
        $domProjects->load($studentChoices);
        ...
        foreach($students as $stTag)
        {
                ...
                $projectsXsltProc->setParameter('','stuid',$stuid);
                $projectsXsltProc->setParameter('','projectChoice',$projectChoice);
                $projectsXsltProc->setParameter('','prefNo',$prefNo);
                $projectsXsltProc->setParameter('','degreecode',$degreecode);
                $projectsXsltProc->setParameter('','studentCount',$studentCount);
                $projectsXsltProc->setParameter('','notSaved',$notSaved);
                ...
                $html.= "<td $colspan>$projectPreferences". $projectsXsltProc->transformToXML( $domProjects )."</td>";
                if ($studentCount==1) 
                {
                    $html.="<td rowspan='100'>";
